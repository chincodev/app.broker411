...
import { fetchData } from 'src/store/apps/report'
    ...
    renderCell: ({ row }) => {
      return (
        <CustomChip
          skin='light'
          size='small'
          label={row.is_checked ? 'CHECKED' : 'PENDING'}
          color={row.is_checked ? 'primary' : 'warning'}
        />
      )
    }
  },
    ...
    renderCell: ({ row }) => {
      return (
        <CustomChip
          skin='light'
          size='small'
          label={row.review.is_enabled ? 'ENABLED' : 'DISABLED'}
          color={row.review.is_enabled ? 'primary' : 'warning'}
        />
      )
    }
  },
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Tooltip title='View Record'>
            <Box>
              <Link href={`/admin/reports/[id]`} as={`/admin/reports/${row.id}`} onClick={()=>{
